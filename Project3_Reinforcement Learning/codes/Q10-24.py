...
# Q13
c_opt, D_opt, b_opt = get_c_D_b(exp_action, P_acts, lam_max, 1.0)
reward_opt = np.array(get_reward(c_opt, D_opt, b_opt))
reward_opt_graph = reward_opt.reshape(10, 10).T
plt.pcolor(reward_opt_graph, cmap = "PuBu_r")
...
# Q14
values = optimal_state_val(values, w, gamma, reward_opt, thres)
g1 = np.round(values, decimals=3).reshape(10, 10).transpose()
plt.figure()
plt.pcolor(g1, cmap = "PuBu_r")
plt.gca().invert_yaxis()
plt.colorbar()
plt.title("Heat Map of Optimal State Values for Extracted Reward Function1")
...
arrows = ['\u2190', '\u2191', '\u2192', '\u2193']
for state in range(100):
    pis[state] = arrows[np.argmax([compute(state, 0, w, gamma, reward_opt, values),
                                   compute(state, 1, w, gamma, reward_opt, values),
                                   compute(state, 2, w, gamma, reward_opt, values),
                                   compute(state, 3, w, gamma, reward_opt, values)])]
...
arrows = ['\u2190', '\u2191', '\u2192', '\u2193']
for state in range(100):
    pis2[state] = arrows[np.argmax([compute(state, 0, w, gamma, reward_1, values),
                                    compute(state, 1, w, gamma, reward_1, values),
                                    compute(state, 2, w, gamma, reward_1, values),
                                    compute(state, 3, w, gamma, reward_1, values)])]
pis1_modified = []
...
print("Optimal policy for extracted reward function 1:")
plt.figure()
...
print("Compare - Optimal policy for extracted reward function 1:")
plt.figure()
...
print("Compare - Optimal policy for ground truth reward function 1:")
plt.figure()
...
plt.pcolor(reward_opt_graph2, cmap = "PuBu_r")
plt.gca().invert_yaxis()
plt.colorbar()
...
# Q21
values = optimal_state_val(values, w, gamma, reward_opt2, thres)
g1 = np.round(values, decimals=10).reshape(10, 10).transpose()
plt.figure()
plt.pcolor(g1, cmap = "PuBu_r")
plt.gca().invert_yaxis()
plt.colorbar()
